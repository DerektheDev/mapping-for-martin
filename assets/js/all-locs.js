...
function displayMeta(args) {
  const { lat, lon, city, region } = args;

  $("#map-lat-coords").html(parseFloat(lat).toFixed(4));
  $("#map-lon-coords").html(parseFloat(lon).toFixed(4));
  $("#map-title").html(city);
  $("#map-subtitle").html(region);
}
